...
const { getRateLimit, updateRateLimit } = require('./lib/rate-limit');
const { createResponse } = require('./lib/api-response');
const { lambdaWrapper } = require('../utils/lambda-wrapper');

const _make-callHandler = async (event, context) => {
  try {
...
exports.handler = lambdaWrapper(_make-callHandler); 