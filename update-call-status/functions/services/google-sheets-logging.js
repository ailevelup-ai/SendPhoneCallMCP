  ...
  try {
    ...
    const values = [
      [
        ...
        data.duration ? (data.duration / 60).toFixed(2) : '0', // Call Duration in minutes
        ...
        data.billed_amount || '0', // Billed Amount
        data.error_message || '', // Error Messages
        data.call_summary || '', // Call Summary
        data.recording_url || '', // Recording URL
        '', // Request Parameters
        '', // Response Parameters
        data.concatenated_transcript || '', // Concatenated Transcript
        data.transfer_number || '', // Transfer Number
        data.answered_by_enabled ? 'true' : 'false', // Answered By
        data.call_ended_by || '', // Call Ended By
        data.analysis_score || '', // Analysis Score/Analysis
        data.update_status || 'Pending', // Computed Durs Update Status
